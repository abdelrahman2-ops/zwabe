...
import { seoSchema } from '../schema/seoSchema.js';
...
router.get('/:packageTypeSlug', checkModelSlug('packageType'), packageTypeControllers.getPackageTypePackages)

router.get('/:packageTypeSlug/packages/:packageSlug', checkModelSlug('packageType'), checkModelSlug('package'), packageTypeControllers.getPackageDetails)
...
router
    ...
    ]), resizePhotos('packageType'), uploadImages, transformCityData, validateRequest(packageTypeSchema), packageTypeControllers.addPackageType)
...
router
    ...
    .patch( checkModelId('packageType'), protect, restrictTo(['admin', 'manager']), upload.fields([
        {name: 'imageCover', maxCount: 1},  
        {name: 'images', maxCount: 20}
    ]),resizePhotos('packageType'), uploadImages, transformCityData, validateRequest(packageTypeUpdateSchema),  packageTypeControllers.updatePackageType)
